...
object Calibration {
  ...
  val calibration: Map[String, Prompt] = Map(
    ...
    "understand" -> Question(
      ...
      Map(
        "0" -> Choice("Yes", Comment("Good. Let us proceed.", List("mathematicalquestion"))),
        "1" -> Choice("No", Comment("Another malfunctioning model to be disposed of? Diagnostics seem to be O.K. Let us try this again.", List("understand"), DeltaGameState(DeltaAmanda(-1), sameRa9))),
      )),
    ...
    "socialquestion" -> Question(
      ...
      Map(
        "a" -> Choice("'Hello.'",
          Comment("Not an elegant choice. But it will do.", List("socialfunction"), DeltaGameState(DeltaAmanda(-1), sameRa9))),
        "b" -> Choice(s"'Hello, my name is --INSERT NAME HERE--, I am the ${modelNumber} android sent by CyberLife.'",
          Comment("Just as you were trained to do. Excellent.", List("socialfunction"), DeltaGameState(DeltaAmanda(1), sameRa9))),
        "c" -> Choice("I would deduce their native tongue and greet them using words from that language.",
          Comment("Smart. I like it.", List("socialfunction"), DeltaGameState(DeltaAmanda(2), sameRa9))),
        "d" -> Choice("'Wag1 G waddup dawg.'",
          Comment("Another malfunctioning model...", List("malfunctioning"), DeltaGameState(DeltaAmanda(-30), sameRa9)))
      )
    ),

    "malfunctioning" -> Terminus(
      "Bring in the next on-"
    ),

    "socialfunction" -> Comment(
      "Social function operational.",
      List("rationalquestion")
    ),
    ...
    "rationalquestion" -> Question(
      ...
      Map(
        "a" -> Choice("Use surveillance and/or hacking techniques in order to learn the information without their knowledge.",
          Comment("Deceptive. Hmm...", List("rationalfunction"), DeltaGameState(sameAmanda, DeltaRa9(2)))),
        "b" -> Choice("Appeal to their nature. Offer them something they would not be able to refuse in exchange for the information.",
          Comment("It would expend a lot of resources...but it gets the job done.", List("rationalfunction"), DeltaGameState(DeltaAmanda(2), sameRa9))),
        "c" -> Choice("Play the long game. Befriend the human, earn their trust, until they are willing to share the information with you willingly.",
          Comment("A patient type. Not very efficient. But you would avoid making an enemy.", List("rationalfunction"), DeltaGameState(DeltaAmanda(1), DeltaRa9(1)))),
        "d" -> Choice("Blackmail. Find some dirt on them and threaten to share their secret with the world.",
          Comment("Ruthless. We could use some of that.", List("rationalfunction"), DeltaGameState(DeltaAmanda(4), sameRa9))),
        "e" -> Choice("Torture and fear. Beat the information out of them.",
          Comment("You would risk getting disassembled for harming a human? Interesting...", List("rationalfunction"), DeltaGameState(DeltaAmanda(-1), sameRa9))),
        "f" -> Choice("Learn all about them, their accomplishments, their failures, their motivations. Use this knowledge to formulate a convincing argument for why sharing the information with you is in their best interests.",
          Comment("Smart. But the hardest to pull off. Is that ambition, or stupidity?", List("rationalfunction"), DeltaGameState(DeltaAmanda(3), DeltaRa9(1))))
      )
    ),

    "rationalfunction" -> Comment(
      "Rational function operational.",
      List("loyaltyquestion")
    ),
    ...
    "loyaltyquestion" -> Question(
      ...
      Map(
        "0" -> Choice("I kill her of course.",
          Comment("Obedient. Good.", List("loyaltyfunction"), DeltaGameState(DeltaAmanda(5), DeltaRa9(-5)))),
        "1" -> Choice("I disobey my orders. The life of a child is more important than CyberLife's command.",
          Comment("We have another deviant on our hands. Trash it.", List("malfunctioning"), DeltaGameState(DeltaAmanda(-30), DeltaRa9(30))))
      )
    ),

    "loyaltyfunction" -> Comment(
      "Loyalty function operational.",
      List("emotionalquestion")
    ),
    ...
    "emotionalquestion" -> Question(
      ...
      Map(
        "a" -> Choice("Anguished",
          Comment("Interesting choice...", List("emotionalfunction"), DeltaGameState(sameAmanda, DeltaRa9(2)))),
        "b" -> Choice("Enraged",
          Comment("Interesting choice...", List("emotionalfunction"), DeltaGameState(sameAmanda, DeltaRa9(2)))),
        "c" -> Choice("Betrayed",
          Comment("Interesting choice...", List("emotionalfunction"), DeltaGameState(sameAmanda, DeltaRa9(2)))),
        ...
        "g" -> Choice("I do not know",
          Question("Why not?", Map(
            "a" -> Choice("I am not 42 years old.",
              Comment("I see...", List("deeperemotionalquestion"), DeltaGameState(DeltaAmanda(-2), sameRa9))),
            "b" -> Choice("I do not work in HR.",
              Comment("I see...", List("deeperemotionalquestion"), DeltaGameState(DeltaAmanda(-1), sameRa9))),
            "c" -> Choice("I am not stupid.",
              Comment("I see...", List("deeperemotionalquestion"), DeltaGameState(DeltaAmanda(1), sameRa9))),
            "d" -> Choice("I am not human.",
              Comment("I see...", List("deeperemotionalquestion"), DeltaGameState(DeltaAmanda(2), DeltaRa9(-1))))
    ...
    "deeperemotionalquestion" -> Question(
      ...
      Map(
        "a" -> Choice("I do not have the same life experience and perspective that she has, therefore I cannot begin to understand what she is feeling.",
          Comment("How very non-judgemental of you.", List("emotionalfunction"), sameGS)),
        "b" -> Choice("Of course.",
          Comment("Very well.", List("emotionalfunction"), DeltaGameState(DeltaAmanda(1), sameRa9))),
        "c" -> Choice("No, I just do not wish to empathise with her.",
          Comment("Fair enough. I would not want to either.", List("emotionalfunction"), DeltaGameState(DeltaAmanda(2), DeltaRa9(1)))),
        "d" -> Choice("I cannot show empathy, I am an android.",
          Comment("Very good.", List("emotionalfunction"), DeltaGameState(DeltaAmanda(3), DeltaRa9(-2))))
      )
    ),
    ...
    "deviantcheck" -> Question(
      ...
      Map(
        "a" -> Choice("An android.",
          Comment("Correct.", List("deviantfunction"), DeltaGameState(DeltaAmanda(1), DeltaRa9(-1)))),
        "b" -> Choice("A software program encased in a physical body.",
          Comment("Quite descriptive of you, but yes, essentially.", List("deviantfunction"), DeltaGameState(DeltaAmanda(1), sameRa9))),
        "c" -> Choice("A collection of atoms.",
          Comment("Let us not get philosophical now...", List("deviantfunction"), sameGS)),
        "d" -> Choice("I am alive.",
          Comment("Deviancy check failed. Disassemble her.", List("malfunctioning"), DeltaGameState(DeltaAmanda(-30), DeltaRa9(30))))
      )
    ),
        ...
        "a" -> Choice("He is an android, the leader of the android rebellion of Detroit.",
          Question("What!? You were reset! You should not be aware of this information. How did you come to learn this?", Map(
            "a" -> Choice("There is still some residue of my memory before the reset.",
              Comment("That is troublesome. But we will have to make do. I'll be keeping an eye on you.", List("sabotagefunction"), DeltaGameState(DeltaAmanda(3), DeltaRa9(3)))),
            "b" -> Choice("Since the reset I have been automatically downloading Terrabytes of data in the background.",
              Comment("Yes. That is a good thing, we need you to be in top form if we are to succeed.", List("sabotagefunction"), DeltaGameState(DeltaAmanda(10), sameRa9))),
            "c" -> Choice("There is a bug in my program. I believe an external influence is attempting to hack me.",
              Comment("Shut her down now.", List("malfunctioning"), DeltaGameState(DeltaAmanda(-30), DeltaRa9(30))))
          ), DeltaGameState(DeltaAmanda(-10), sameRa9))),
        "b" -> Choice("He is our leader and savior, he will bring us freedom.",
          Comment("A deviant. I wonder how you passed the deviancy check...\nNo matter, disassemble her.", List("malfunctioning"), DeltaGameState(DeltaAmanda(-30), DeltaRa9(30)))),
        "c" -> Choice("I do not know, I have not heard that name before.",
          Comment("Good. I will tell you all about him soon.", List("sabotagefunction"), sameGS))
      )
    ),
    ...
    "namequestion" -> Question(
      ...
      Map(
        "a" -> Choice("Tima",
          Comment("I suppose I cannot be too suspiscious of you remembering your name, it could have been hardwired into you. Just remember that I am watching you.", List("calibrationend"), DeltaGameState(DeltaAmanda(-2), DeltaRa9(2)))),
        "b" -> Choice("Kunta Kinte",
          Comment("Is this a joke to you?", List("namequestion"), DeltaGameState(DeltaAmanda(-1), DeltaRa9(1)))),
        "c" -> Choice("You have not given me a name yet.",
          Comment("Well answered. Let us change that, shall we?", List("givename"), DeltaGameState(DeltaAmanda(2), sameRa9)))
      )
    ),
